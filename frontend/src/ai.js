...
export function initAIRouteBuilder() {
    ...
    generateBtn.addEventListener("click", async () => {
        ...
        try {
            const result = await callFunction('generateItinerary', { prompt: prompt });

            if (result.roteiroId) {
                 window.open(`/roteiro_publico.html?user=${user.uid}&id=${result.roteiroId}`, '_blank');
                 if(resultText) resultText.innerHTML = '<p class="text-green-400 text-center">Seu roteiro foi gerado em uma nova aba!</p>';
            } else {
                throw new Error(result.error || "ID do roteiro não foi retornado pela função.");
            }

        } catch (error) {
            console.error("Erro ao gerar roteiro:", error);
            if(resultText) resultText.innerHTML = `<p class="text-red-400 text-center">Ocorreu um erro: ${error.message}</p>`;
            // O showAlert já é chamado pelo apiService
        } finally {
